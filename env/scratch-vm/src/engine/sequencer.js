...
class Sequencer {
     ...
     * @param {?number} minimumStack The stack level at which steps will no longer be taken (if present).
     * @returns {boolean} if minimum stack was specified, whether or not it was required for usage.
     */
    stepThread (thread, minimumStack) {

        if(!minimumStack) minimumStack = Number.NEGATIVE_INFINITY;
        let currentBlockId = thread.peekStack();

        if (!currentBlockId) {
            if(thread.stack.length === minimumStack) {
                return true;
            }
